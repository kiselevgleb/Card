...
export default function findPaySys(num) {
  let testData = "null"
  let corrent = luhnAlgorithm(num);
  document.body.querySelectorAll("span").forEach(element => {
    element.classList.remove('cdisabled');
  });
  pPinned.innerText = '';
  if (corrent) {
    let firstChar = num.substring(0, 1);
    let firstAndSec = num.substring(0, 2);

    switch (firstChar) {
      case '2':
        document.body.querySelectorAll("span").forEach(element => {
          if (element.classList.contains("mir")) {testData = "mir"} else {
      ...
      case '3':
        if (firstAndSec === "30" || firstAndSec === "36" || firstAndSec === "38") {
          document.body.querySelectorAll("span").forEach(element => {
            if (element.classList.contains("din")) {testData = "din"} else {
              element.classList.add('cdisabled');
            }
          });
        } else if (firstAndSec === "31" || firstAndSec === "35") {
          document.body.querySelectorAll("span").forEach(element => {
            if (element.classList.contains("jcb")) {testData = "jcb"} else {
              element.classList.add('cdisabled');
            }
          });
        } else if (firstAndSec === "34" || firstAndSec === "37") {
          document.body.querySelectorAll("span").forEach(element => {
            if (element.classList.contains("amer")) {testData = "amer"} else {
              element.classList.add('cdisabled');
            }
          });
        } else {
          pPinned.innerText = 'Error1';
          console.log(firstAndSec);
        }
        break;
      case '4':
        document.body.querySelectorAll("span").forEach(element => {
          if (element.classList.contains("visa")) {testData = "visa"} else {
            element.classList.add('cdisabled');
          }
        });
        break;
      case '5':
        if (firstAndSec === "51" || firstAndSec === "52" || firstAndSec === "53" || firstAndSec === "54" || firstAndSec === "55") {
          document.body.querySelectorAll("span").forEach(element => {
            if (element.classList.contains("master")) {testData = "master"} else {
              element.classList.add('cdisabled');
            }
          });
        } else {
      ...
      case '6':
        if (firstAndSec === "60") {
          document.body.querySelectorAll("span").forEach(element => {
            if (element.classList.contains("dis")) {testData = "dis"} else {
              element.classList.add('cdisabled');
            }
          });
        } else {
...
butInput.addEventListener('click', (event) => {
  let numb = pInput.value;
  findPaySys(numb);
});